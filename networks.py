...
def define_s(gpu_ids=[], skip=False, opt=None):
    nets = None
    ...
    if len(gpu_ids) > 0:
        ...
        nets = torch.nn.DataParallel(nets, gpu_ids)
    return nets
...
def define_f(gpu_ids=[], skip=False, opt=None):
    ...
    netf = FusionNet(opt, skip)

    if len(gpu_ids) > 0:
        netf.cuda(device=gpu_ids[0])
        netf = torch.nn.DataParallel(netf, gpu_ids)
    return netf
